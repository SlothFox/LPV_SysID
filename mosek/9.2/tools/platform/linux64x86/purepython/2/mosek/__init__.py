...
__library__,__dlldir__ = loadmosek("mosekxx9_2")
...
__library__.MSK_XX_putoptserverhost.restype  = ctypes.c_int
__library__.MSK_XX_putoptserverhost.argtypes = [ctypes.c_void_p,ctypes.c_char_p]
...
class Task:
  ...
  @accepts(_accept_any,_accept_str)
  @synchronized
  def putoptserverhost(self,host_):
    """
    Specify an OptServer for remote calls.
  
    putoptserverhost(self,host_)
      host: str|unicode. A URL specifying the optimization server to be used.
    """
    if isinstance(host_,unicode):
      host_ = host_.encode("utf-8",errors="replace")
    res = __library__.MSK_XX_putoptserverhost(self.__nativep,host_)
    if res != 0:
      _,msg = self.__getlasterror(res)
      raise Error(rescode(res),msg)
